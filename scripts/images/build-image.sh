#!/usr/bin/env bash
# build-image.sh - Build debthin container images using mmdebstrap
#
...
#
# Usage:
#   ./build-image.sh debian/trixie/amd64
#   ./build-image.sh ubuntu noble arm64
...
# --- Cleanup trap ---
cleanup() {
    if [ -n "${ROOTFS_MNT:-}" ] && [ "${ROOTFS_MNT:-}" != "/" ]; then
