...
// Pre-built manifest matching the structure generated by generate_image_manifest.py
...
const mockEnv = {
    IMAGES_BUCKET: {
        ...
        async get(key) {
            if (key === "images-manifest.json") {
                getCallCount++;
                const buf = new TextEncoder().encode(MOCK_MANIFEST);
                return {
...
test('images/LXC Index Generation (Caching logic)', async () => {
    ...
    // Call 1: Miss, Generates text (or HIT if the background thread pre-warms first)
    ...
    assert.equal(getCallCount, 1); // Fetched manifest once
...
test('images/Incus Images Index (Concurrency coalescing)', async () => {
    ...
    // Despite 3 requests, manifest was fetched only 1 time
    assert.equal(getCallCount, 1);
});
